# ANTIC Display Processor

Router for ANTIC display lists, DMA timing, scrolling, registers, and display-list interrupts.

Load only the focused file that matches the task.

- [overview.md](antic/overview.md) - Quick-lookup
- [dma-timing.md](antic/dma-timing.md) - 3.1  ANTIC Architecture, DMA & Timing
- [display-list-instructions.md](antic/display-list-instructions.md) - 3.2  Display List Instructions (Bitmask Encoding)
- [scrolling.md](antic/scrolling.md) - 3.3  Scrolling — Coarse & Fine (2D, Hardware-Accelerated)
- [character-modes.md](antic/character-modes.md) - 3.4  Character Modes (ANTIC 2–7)
- [registers.md](antic/registers.md) - 3.5  ANTIC Register Quick-Reference ($D400–$D40F)
- [dli-wsync.md](antic/dli-wsync.md) - 3.6  DLI — Full Reference
- [notes.md](antic/notes.md) - 3.11  Emulator Accuracy Notes
