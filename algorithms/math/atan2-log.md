...
; input:  x1, x2, y1, y2 (screen coords, unsigned byte positions)
; output: A = atan2(y1-y2, x1-x2) in 256-degree circle
; destroys: X, Y, atan_octant
...
atan2
    ...
    eor #$ff             ; take |dx|
    tax
    rol atan_octant      ; fold x sign/ordering bit into octant
    ...
    eor #$ff             ; take |dy|
    tay
    rol atan_octant      ; fold y sign/ordering bit into octant
...
The `atan_tab` encodes `atan(2^(n/32))*128/pi` and `octant_adjust` holds 8
